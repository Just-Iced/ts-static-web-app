...
export async function getCount(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    // Execute a query
    const result = await pool.request().query("SELECT COUNT(*) AS number FROM Students");
    const content = `There are ${result.recordset[0].number} students as of ${new Date()}`;
